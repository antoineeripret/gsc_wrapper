...
If you are not using the API, you can load a `Report` object from any DataFrame using the following code: 
...
(
    ...
    .query
    .from_dataframe(
        #dataframe where your data are stored 
        df, 
        #webproperty object 
        webproperty
    )
...
- [add_first_found](#add_first_found())
...
Please note that there are some checks done for the API methods to ensure required dimensions are included. 
...
|Required dimensions|Required metrics| Output|
|:----|:----|:----|
|query / page | None |pd.DataFrame|

This method is used to return the number of (unique) queries per page. More information [here](https://www.searchenginejournal.com/big-query-and-gsc-data-content-performance-analysis/508481/). 
...
|url|uqc|
|:----|:----|
|https://www.example.com/page1|4598|
|https://www.example.com/page2|3456|
...
When you deal with large website, you often analyze data by page type and not individually. To achieve this goal, you have two options:
- Filter your GSC data using the `page` dimension with the `filter` method. This works but can be time-consuming if your project counts with dozens of page types. 
- Use this method 

It can be used using this snippet: 
...
It will return a copy of `report` with a new column `category` that you can use in your analysis. Please note that the `rules` object must be a Pandas Dataframe with the following columns: 
- `type` that indicates the kind of rule you have. The accepted values are: `equals`, `contains` and `includingRegex`. If you use a REGEX, be sure to test in the GSC UI first. 
- `rule` with the value that will be used with the `type` column to create the condition 
- `category` with the name of your page type 

For example: 

|type|rule|category|
|:----|:----|:----|
|equals|https:///www.example.com/|homepage|
|contains|product|PDP|
|includingRegex|[0-9]+|categories|

If you have more than condition that is met for a given row, **the first one will be applied**. Once the method has been applied, you can execute the following code and ensure that the values returned are what you expect: 
...
Under the hood this method uses the [case_when() method from Pandas](https://pandas.pydata.org/docs/reference/api/pandas.Series.case_when.html). 

### add_first_found()

|Required dimensions|Required metrics| Output|
|:----|:----|:----|
|page (or query) and date | None |pd.DataFrame|

This method will add a new column, `first_found_page` or `first_found_query`, to the underlying dataframe your report has. It will analyze your data and tell you when a given page or query has been seen for the first time. **Please be aware that this method is obviously limited by the data in your report, hence be sure to use it only when you have a decent amount of days, otheriwse the findings would be meaningless.**

You can easily cann this method: 

```python 

(
    report 
    #the parameter can be either page or query here 
    .add_first_found("page")
    .show_data()
)

```

It will return the same `Report` object, but if you run 
...
It will include a new column, `first_found_page` or `first_found_query`, with the date. This data could allow you to visualize new keywords / page in your dataset to gain additional insights from your organic search traffic, such as the impact of new content creation. 
...
|Required dimensions|Required metrics| Output|
|:----|:----|:----|
|query | clicks / impressions |pd.DataFrame|

This method allows us to understand what are the most common keyword structures used in our `Report`. Its usage is quite simple: 
...
( 
    ...
    .word_frequency(
        phrase_len=1, 
        stopwords=gscwrapper.stopwords['english'], 
        rm_words=[]
    )
)
...
- The `phrase_len` parameter helps you decide if you want to analyze unigrams, bigrams, trigrams .... By default, the value is always `1`. 
- The `stopwords` allows you lo load a seto of stopwords for most common languages. Have a look at [this file](https://github.com/antoineeripret/gsc_wrapper/blob/master/gscwrapper/stopwords.py) for the list of all available languages. By defauly, it will load the `english` dictionnary. 
- The `rm_words` parameter allows you to complete the default list of stop words by a custom one. Useful if one of your stop words is not being taken into account. 

This method will return a list of (for instance) unigrams with the number of times they have been spotted amongst your keyword list and the sum of clicks / impressions. For example: 


|keyword|count|clicks|category|
|:----|:----|:----|:----|
|apple|12|123|45678|
|banana|21|456|97932|
|strawberry|34|987|74847|
...
This method allows you to retrieve response codes (using the [request library](https://pypi.org/project/requests/)) for the pages included in your `Report`. While I don't advise to use this method if you have a lot of URLs (the code is not designed for larg-scale crawling), it can save time if you don't have a lot. 
...
This method will return a copy of the `Report` object, appending a new column with the response codes. Please note that the `wait_time` (in seconds) parameter is optional and is added to ensure that you can crawl a website that may block you if you crawl it too fast.  